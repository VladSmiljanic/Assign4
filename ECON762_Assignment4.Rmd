...
\begin{align*}
(X_{(-t)}X'_{(-t)})^{-1} &= \frac{(X'X)^{-1} + (X'X)^{-1}x_tx'_t(X'X)^{-1}}{(1 -h_{tt})}\\
\end{align*}

c) Similarily, $X'Y = \sum^n_{i=1}x_iy'_i$. We can find $X'_{(-t)}Y_{(-t)}$ by subtracting the $t^{th}$ observation $x_ty'_t$:

\begin{align*}
X'_{(-t)}Y_({-t}) = X'Y - x_ty'_t\\
\end{align*}
